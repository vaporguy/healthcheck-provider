...
public final class Provider: Vapor.Provider {
	...
	public init(config: Config) throws {
		if let healthCheckUrl = config["healtcheck", "url"]?.string {
	...
	public func boot(_ drop: Droplet) {
		guard let healthCheckUrl = self.healthCheckUrl else {
			return drop.console.warning("MISSING: healthcheck.json config in Config folder. Healtcheck URL not added.")
			}
